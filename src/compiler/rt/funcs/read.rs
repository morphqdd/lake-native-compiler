...
const SYSCALL_WRITE: i64 = 0; // Linux x86-64

/// Build `rt_read(fd, fat_ptr, size)` — bounds-checked write syscall.
pub fn define_read(mut ctx: CompilerCtx) -> Result<CompilerCtx> {
    ...
    let id = ctx
        .module_mut()
        .declare_function("rt_read", Linkage::Export, &sig)?;
